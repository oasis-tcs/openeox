...
- Registration and Quorum Calculation
    - Thomas proposes using an open source tool that will track attendance and voting status for TC members given the current challenges with OASIS platform.
      - Received verbal permission from OASIS to use the open source tool. Awaiting for written approval.
      - Thomas asks TC if we would like to use the open source tool for quorum calculation and voting status tracking?
    ...
    - Omar states that an open source tool that is contributed by an organization or idividual that is going to be continued to be maintained, that would be welcomed by OASIS.
    - Omar mentions that Stefan and Thomas are part of the OASIS Board Technology Subcommittee currently discussing solutions for OASIS.
    - Omar asks Thomas if the open source tool will be integrated with the OASIS infrastructure registration?
        - Thomas clarifies that no the open source tool is not currently integrated. If the OASIS Board Technology Subcommittee decides to use the tool, they can choose to integrate it into the OASIS workflow.
        - Thomas says at this time it does not make sense to bind the tool to the OASIS infrastructure.
    - Omar mentions that not only is attendance and quorum calculation/voting status a problem, but also the roster and current status of participation of members/organizations.
    - Thomas suggests using the tool as an interim solution and to track attendance, quorum, voting status independently for the time being.
    - Justin confirms support for Thomas' suggestion re: the open source tool for quorum calculation.
- In Person Meeting
    - Thomas suggest having an in person meeting to help make more significant progress with OpenEoX.
    - Omar suggests RSA as a possible venue for in person meeting.
    - Thomas and Justin discuss inability to travel to RSA due to administration transition and budgeting changes. Thomas suggests VulnCon.
    - Justin also suggests VulnCon and supports the idea of an in person meeting, but suggests that we make it a hybrid meeting to allow optimized participation from the TC.
    ...
    - In person meeting will take place tentatively on Wednesday April 9th at 3 PM Eastern Time at Cisco RTP office.
        -  - Thomas suggests we meet Wednesday and Thursday.
- Other External Efforts re: End-of-Life
    ...
    - Denny empasized the need to move quick.
    - Shridhar states that while we need to operationalize the effort and make the standard more robust, and that he is not a fan of moving too quickly that we miss something.
    - Rogue suggests that this group should define a minimum what we want to use as a starting point and extend it and add new features based on the requirements.
    - Thomas mentions the Ecma TC54 TG3 CLE standard (Issue [#71](https://github.com/oasis-tcs/openeox/issues/71)) and confirms that we are not the only effort, but that we have two solid definitions with End-of-Life and End-of-Sales.
    ...
    - Rogues mentions that with SPDX and CycloneDX there are already fields where you specify.
...
- PR [#69]((https://github.com/oasis-tcs/openeox/pull/69)
    ...
    - Justin seconded. No objections. No discussion. Motion passed.
